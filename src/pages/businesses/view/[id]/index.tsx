...
import { deleteCakeDesignById } from 'apiSdk/cake-designs';
...
function BusinessViewPage() {
  ...
  const cake_designHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
              ...
              {hasAccess('cake_design', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/cake-designs/create?business_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.cake_design?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/cake-designs/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>{record.name}</Td>
                            ...
                            <Td>
                              {hasAccess('cake_design', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/cake-designs/edit/${record.id}`}
                                >
                              ...
                              {hasAccess('cake_design', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    cake_designHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
