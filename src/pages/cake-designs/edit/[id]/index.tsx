...
import { getCakeDesignById, updateCakeDesignById } from 'apiSdk/cake-designs';
...
function CakeDesignEditPage() {
  ...
  const handleSubmit = async (values: CakeDesignInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateCakeDesignById(id, values);
      mutate(updated);
      resetForm();
      router.push('/cake-designs');
    } catch (error) {
      setFormError(error);
    }
  };

  const formik = useFormik<CakeDesignInterface>({
    initialValues: data,
    validationSchema: cakeDesignValidationSchema,
